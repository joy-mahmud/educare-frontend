...
export default function TeacherLogin() {
    ...
    try {
      ...
      if (err.response && err.response.status === 400) {
        Swal.fire({
          ...
          text: "Invalid teacherId or password. Please register first if are not registered yet.",
  ...
  const handleRegisterSubmit = async () => {
    if (registerData.password !== registerData.confirmPassword) {
      return setError("Please use the same password in both fields");
    }
    const data = {
    ...
    try {
      const res = await axios.post(
        `${BASE_URL}/api/auth/teacher-registration/`,
        data
      );
      if (res.status === 201) {
        setSubmitted(true);
  ...
  return (
    <div
      style={{ background: "" }}
      className="mx-20 bg-gray-300 min-h-screen flex items-center justify-center p-4"
    >
